...
startLoop / endLoop | `index(Number)` | startLoop(11) | Explicit function for using the jump economizer extension that reads in values at `index` from a table. Valid values are 11-41. Backend code automatically deals with nested loops for inner (R1) vs outer (R2) option lines.
